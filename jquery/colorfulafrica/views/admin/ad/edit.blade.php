...
@section('title', '编辑广告管理')
@section('content')
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title">编辑广告管理</h3>
        </div>
        <form action="/backstage/adinfo/edit-ad" class="form-horizontal" method="post" m-bind="ajax" id="formV" >
            ...
            <div class="box-body">
                    ...
                    <div class="col-sm-2">
                        <select name="type" id="cate" class="form-control">
                            ...
                            @foreach($cate as $key=>$cate)
                                <option value="{{$key}}"@if($key==$detail['type']) selected @endif>{{$cate}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>

                <div class="form-group" id="info"  @if($detail['type']==7) style="display:none" @endif>
                    <label for="username" class="control-label col-sm-2 required">详情</label>
                    <div class="col-sm-2">
                        <select name="resourceId"  class="form-control">
                        <option value="">---请选择---</option>
                        @if($detail['type']!=7)
                        @foreach($sec as $key=>$sec)
                            <option value="{{$sec['id']}}"@if($sec['id']==$detail['resourceId']) selected @endif>{{$sec['name']}}</option>
                        @endforeach
                        @endif
                         </select>
                    </div>
                ...
                <div class="form-group">
                    <label for="username" class="control-label col-sm-2 required">跳转链接</label>
                    <div class="col-sm-4">
