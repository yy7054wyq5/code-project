	...
	'mF_fscreen_tb':function(settings,$){
		...
		var $thumbUl=$thumbBox.find('ul');
  		var $thumbList=$thumbUl.find('li');
            ...
            //var $sizeList = document.getElementById('sizeLi').getElementsByTagName('li');
            //console.log($sizeList[0]);
		// var $prevBtn=$focus.addHtml('<div class="prev"><a href="javascript:;">&#8249;</a></div>');原样式
		// var $nextBtn=$focus.addHtml('<div class="next"><a href="javascript:;">&#8250;</a></div>');原样式
		...
		// $thumbBox.find('img').each(function(){this.style.height=(p.thumbHeight-9*2)+'px';});//8px margin+1px border //lanwang
		//$txtList.each(function(){this.style.left=-settings.width+'px'});'

		//为联动准备参数
		for (var i = 0; i < $thumbList.length; i++) {
			$thumbList[i].setAttribute('index',i);
		};
