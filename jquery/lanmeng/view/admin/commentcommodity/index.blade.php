        ...
        <div class="box-body">
            <div id="datatable1_wrapper" class="dataTables_wrapper form-inline" role="grid">
                <div class="row">
                    <div class="col-sm-12">
                        <div class="pull-left">
                            <form action="/superman/commoditycomment/index" method="get">
                                <div class="dataTables_filter" id="datatable1_filter">
                                    <label>评论内容关键:</label>
                                    <label>
                                        <input type="text" name="keyword" aria-controls="datatable1" placeholder="请输入评论内容关键" class="form-control input-sm" value="<?php echo isset($_GET['keyword']) ? $_GET['keyword'] : ""; ?>">
                                    </label>
                                    <label>状态:</label>
                                    <label>
                                        <select  name="status"  class="form-control input-sm" >
                                            <option value="-1" >请选择</option>
                                            <option value="0" {!!   \Illuminate\Support\Facades\Input::get('status') == 0 ? "selected" : "" !!} >未审核</option>
                                            <option value="1" {!!   \Illuminate\Support\Facades\Input::get('status') == 1 ? "selected" : "" !!} >审核未通过</option>
                                            <option value="2" {!!   \Illuminate\Support\Facades\Input::get('status') == 2 ? "selected" : "" !!}>审核通过</option>
                                        </select>
                                    </label>
                    ...
                    @if (count($lists)>0)
                        @foreach ($lists as $value)
                            <tr>
                                ...
                                <td>{{$value -> comment}} </td>
                                <td><select  name="status"  id="satus" data-id="{{$value -> id}}"  >
...
<script type="text/javascript">
    $(function(){
        ...
        $("#satus").change(function(){
           ...
           var id     = $(this).attr('data-id');
            $.ajax({
                type: "POST",
                url:"/superman/commoditycomment/changestatus",
    ...
    function store()
    {
        $.ajax({
            type: "POST",
            url:"/superman/commoditycomment/del",
            data:$('#form').serialize(),
            dataType: 'json',
            success: function(msg) {
                layer.msg(msg['tips'])
    ...
    function reload()
    {
        location.href = '/superman/commoditycomment/index';
    }
</script>
@stop