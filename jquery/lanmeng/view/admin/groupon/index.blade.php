                            ...
                            @if (count($lists)>0)
                                @foreach((array)$value['sub'] as $v)
                                    <tr>
                                        ...
                                        <td>{{ $v->priceRange }}</td>
                                        <td>{{ $v->payMethod}}</td>
                                        <td>@if(in_array('edit', $buttonrole))<a href="/superman/mall/edit/{{$v->categoryId}}" class="btn btn-success btn-xs">修改</a>@endif
                                            @if(in_array('del', $buttonrole))<a href="/superman/mall/del/{{$v->categoryId}}" class="btn btn-danger btn-xs">删除</a>@endif</td>
                                    </tr>
                                @endforeach
                            @endif
                        @endforeach
