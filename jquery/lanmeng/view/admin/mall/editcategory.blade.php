    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="box border primary">
                <div class="box-title">
                    <h4><i class="fa fa-table"></i>修改商品分类</h4>
                ...
                <div class="box-body">
                    <form class="form-horizontal" id="form" onsubmit="return false;" >
                        ...
                        <input type="hidden" name="mallCategoryId" value="{{ $detail['id'] }}">
                            ...
                            <div class="col-sm-4">
                                <select name="parentId" id="parentId" class="form-control"  >
                                    ...
                                    @foreach($category as $item)
                                        <option value="{{$item->id}}" @if($detail['parentId'] == $item->id) selected @endif >{{$item->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <input type="text" name="sort"  class="form-control"   value="{{$detail['sort']}}"/>
                            </div>
                            <div class="col-sm-4">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <div class="itinerary form-inline periodprice">
                                    ...
                                    @if(array_get($detail, 'rangePrices'))
                                        @foreach($detail['rangePrices'] as $rangePrice)
                                    <div class="item periodprice">
                                        <div class="input-group input-group-sm">
                                            <input type="hidden" name="rangePriceId[]" class="form-control"  value="{{$rangePrice['mallCategoryRangePriceId']}}">
                                            <input type="text" name="rangeStart[]" class="form-control" placeholder="起始价(包含)" value="{{$rangePrice['rangeStart']}}">
                                            <div class="input-group-addon">至</div>
                                            <input type="text" name="rangeEnd[]" class="form-control" placeholder="截止价(包含)" value="{{$rangePrice['rangeEnd']}}">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <select name="payMethod" class="form-control"  >
                                        <option value="1" @if($detail['payMethod'] == 1) selected @endif >蓝深-展厅家具</option>
                                        <option value="2" @if($detail['payMethod'] == 2) selected @endif >旅游产品-悠迪</option>
                                        <option value="3" @if($detail['payMethod'] == 3) selected @endif >其他的产品-祎策</option>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="form-group specadd">
                            ...
                            @for($i=0;$i<1;$i++)
                                @if(array_get($detail, 'mallAds') && array_get(array_get($detail, 'mallAds'), $i)) {{--*/ $mallAd = $detail['mallAds'][$i] /*--}} @else {{--*/ $mallAd =[] /*--}} @endif
                                <div class="form-group spec" data-index="{{$i+1}}">
                                    <label for="" class="control-label col-sm-2 required"></label>
                                    <div class="col-sm-6">
                                        <div class="input-group">
                                            <div class="input-group-addon">链接</div>
                                            <input type="text" class="form-control" name="link[]" value="{{array_get($mallAd, 'link')? $mallAd['link']:''}}">
                                            <input type="hidden"  name="categoryAdId[]" value="{{array_get($mallAd, 'mallCategoryAdId')? $mallAd['mallCategoryAdId']:''}}">

                                            <div class="input-group-addon specimage1">链接单图
                                                <div class="t1" id="pickimage{{$i+1}}">选择单图</div>
                                            </div>
                                            <input type="hidden" id="adimage{{$i+1}}" name="adimage[]" value="{{array_get($mallAd, 'imageId')? $mallAd['imageId']:''}}">
                                        </div>
                                        <div id="adimageList{{$i+1}}" class="t2">
                                            @if(array_get($mallAd, 'imageId'))
                                                <div class="file-item upload-state-done"><img width="150" height="200" src="/superman/image/get/{{array_get($mallAd, 'imageId')}}"></div>
                                            @endif
                                        </div>
                                    </div>
                                </div>
...
//               displayObj.after("<input type='hidden' name='image' value='"+respone.url+"'/>");
    ...
    function store()
    {
        $.ajax({
            type: "POST",
            url:"/superman/mall/editcategory",
            data:$('#form').serialize(),
            dataType: 'json',
            success: function(msg) {
                layer.msg(msg['tips'])
