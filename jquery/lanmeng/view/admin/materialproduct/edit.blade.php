    ...
    <div class="box border primary">
        <div class="box-title">
            <h4>
                ...
                <span class="hidden-inline-mobile">修改官方素材产品</span>
            </h4>
        </div>
        <div class="box-body">
            <div class="tab-content">
                <div class="tab-pane fade in active" id="box_tab1">
                    ...
                    <form class="form-horizontal" id="form" onsubmit="return false;">
                        ...
                        <input type="hidden" name="materialId"  value="{{ $lists->materialId }}">
                                ...
                                <select class="form-control" id="brand" onchange="getCar();" name="brandId">
                                    ...
                                    @if(isset($brandList) && count($brandList)>0)
                                        @foreach($brandList as $key => $value)
                                            <option value="{{$key}}" data-id="{{$key}}"  {!! $lists->brandId == $key ? 'selected' : ''  !!} >{{$value}}</option>
                                        @endforeach
                                     @endif
                                </select>
                            </div>
                        ...
                        <div class="form-group">
                            <label class="col-sm-2 control-label">车型选择</label>
                            <div class="col-sm-4">
                                <div class='hero-multiselect'>
                                    <select multiple  id="car" name="info[carmodleId][]">
                                        @if(isset($lists->carmodleId) && count($lists->carmodleId)>0 )
                                            @foreach($lists->carmodleId as $key => $value)
                                                <option value="{{$value}}"  selected  >@if(isset($carmodelList[$value]) && count($carmodelList[$value])>0){{$carmodelList[$value]}} @endif </option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <select name="categoryId" class="form-control"  >
                                    @if(isset($secandMater))
                                      <option selected value="{{$secandMater['categoryId']}}" >{{$secandMater['categoryName']}}</option>
                                    @endif
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <input type="text" name="materialName" value=" @if(isset($lists->materialName)){{$lists->materialName}} @endif "   class="form-control" />
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-2 control-label">排序</label>
                            <div class="col-sm-4"><input type="text" name="sort" class="form-control" value="@if(isset($lists->sort)){{$lists->sort}} @endif " /></div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-2 control-label">下载所需积分</label>
                            <div class="col-sm-4">
                                <input type="text" name="integral" value=" @if(isset($lists->integral)) {{$lists->integral}} @endif "   class="form-control" />
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                    <div id="uploader-demo">
                                        ...
                                        <input type="hidden" name="albumpics"   id="albumpics" value="@if(count($lists->imageId)>0){{implode(',',$lists->imageId)}} @endif " />
                                        <div id="albumList" class="uploader-list">
                                            @if(isset($lists->imageId) && !empty($lists->imageId))
                                                @foreach($lists->imageId as $image)
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <div id="uploader"  class="wu-example">
                                    ...
                                    <a style="margin-bottom:0;border:1px solid #ccc; background: #e8e8e8;display: block; width: 100px; height: 50px;" id="upload_file_btn"></a><span class="tips"></span>
                                    <input type="hidden" name="fileId"  value="@if(isset($lists->enclosure)){{$lists->enclosure}} @endif "  id="fileID1"/>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <select name="status" class="form-control">
                                    <option value="0" {{$lists->status == 0 ? "selected" : ""}} >未审核</option>
                                    <option value="1" {{$lists->status == 1 ? "selected" : ""}} >审核通过</option>
                                    <option value="2" {{$lists->status == 2 ? "selected" : ""}} >审核未通过</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <textarea id="container" name="describle" style="width:800px;;height:500px;">{{$lists->describle}}</textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-2 control-label"></label>
                            <div class="col-sm-4">
                                <button id="button" onclick="store();" class="btn btn-primary">保存</button>
...
//               displayObj.after("<input type='hidden' name='image' value='"+respone.url+"'/>");
        ...
        function store()
        {
            $("#button").attr("disabled", true);
                $.ajax({
                    type: "POST",
                    url:"/superman/materialproduct/edit",
                    data:$('#form').serialize(),
                    dataType: 'json',
                    success: function(msg) {
                        layer.msg(msg['tips'])
