...
<div class="box border primary">
    <div class="box-title">
        <h4><i class="fa fa-table"></i>车友订单</h4>
    ...
    <div class="box-body">
        <div id="datatable1_wrapper" class="dataTables_wrapper form-inline" role="grid">
            <div class="row">
                <div class="col-sm-12">
                    <div class="pull-left">
                        <form action="/superman/order/car" method="get">
                        <div class="dataTables_filter" id="datatable1_filter">
                            ...
                            <label>
                                <input type="text" style="width: 80px;" name="account" aria-controls="datatable1" placeholder="账号" class="form-control input-sm" value="{{ Input::get('account') }}">
                            </label>
                            <label>
                    ...
                    @endif
                    &nbsp;&nbsp;
                    <button class="btn btn-sm btn-info" onclick="showDate('/superman/order/exportexcel/2')">导出Excel</button>
            ...
            <table id="datatable1" cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered table-hover dataTable" aria-describedby="datatable1_info">
                ...
                <tbody role="alert" aria-live="polite" aria-relevant="all">
                    ...
                    <td>{{ $value->ointegral / 50 }}</td>
                    <td>{{ $value->pay }}</td>
                    <td>{{ $value->status }}</td>
                    <td>{{ date('Y-m-d H:i', $value->ocreated) }}</td>
                    <td>@if(in_array('edit', $buttonrole))<button class="btn btn-xs btn-info" @if($value->statuscode == 0 && $value->paycode == 0) disabled @endif onclick="payconfirm('{{ $value->ordersn }}');">发货</button>@endif&nbsp;<a class="btn btn-success btn-xs" href="/superman/order/show/{{ $value->ordersn }}">详细</a></td>
                    </tr>
                    @endforeach
                    @else
                    <tr>
