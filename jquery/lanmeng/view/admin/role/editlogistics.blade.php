<div class="form-group add-logistics " style="display: none">
    ...
    <div class="col-sm-4">
        <input type="text" name="logistics[username]" class="form-control" value="{{$info->username}}" placeholder="请输入用户名称">
    </div>
</div>
<div class="form-group add-logistics" style="display: none">
...
<div class="form-group  add-logistics " style="display: none">
    ...
    <div class="col-sm-4">
        <select name="logistics[realname]" class="form-control"   >
            ...
            <option @if($info->realname == $key) selected @endif value="{{ $key }}">{{ $value }}</option>
            @endforeach
        </select>
    </div>
</div>
